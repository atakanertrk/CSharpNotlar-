...
namespace ConsoleApp.WithGenerics
{
    public static class GenericTextFileProcessor
    {
        // Generic Method
        // return type is a list of T
        // T is a any kind of type (example: int, string, *Person, *LogEntry...)
        // With this method, we are saying we accept all types but there is a 
        // limitation: "where T : class, new()" that means; 
        // FirstLimitation: T has to be class (cant be int, string etc..)
        // SecondLimitation: You have to have the ability to have an *empty constructor*.(line 21: T entry = new T();)
        // also: we could add more than one type like: "LoadFromTextFile<T,U>"
